...
test("ships four interactive WebGL halls and twelve concepts", async () => {
  ...
  assert.match(museum, /switchHall\(1\)/);
  assert.doesNotMatch(museum, /handleHallWheel|handleMuseumWheel|wheelAccumulator/);
  assert.doesNotMatch(museum, /window\.addEventListener\("wheel"/);
  ...
  assert.doesNotMatch(museum, /museumAction = "enter-first-hall"/);
  assert.doesNotMatch(museum, /function makeChevronMaterial/);
  assert.doesNotMatch(museum, /entranceGuide|entranceArrow/);
  ...
  assert.match(museum, /vec3 softened = mix\(desaturated, color, 0\.72\) \* 0\.42/);
  ...
  assert.match(museum, /artworkProgress \* Math\.PI \* 2/);
  ...
  assert.match(museum, /const minimumFrameInterval = renderProfile\.targetFps === 30 \? 1000 \/ 30 : 0/);
  ...
  assert.match(garden, /container\.dataset\.quality=lowPower\?"eco":"adaptive"/);
  ...
  assert.doesNotMatch(css, /\.atrium-artwork-carousel/);
  assert.match(page, /getAutoRenderProfile\(\)\.lowDetail/);
  assert.match(page, /premium-nav-glass/);
  assert.match(css, /Performance-aware premium glass navigation/);
  assert.match(css, /\.premium-nav-glass \.site-header\{[^}]*backdrop-filter:blur\(22px\) saturate\(1\.35\)/);
  ...
  assert.match(page, /PRIMARY_SECTION_IDS = \["hall", "workshop", "garden", "hometown"\]/);
  assert.match(page, /const handlePrimaryWheel/);
  assert.match(page, /window\.addEventListener\("wheel", handlePrimaryWheel, \{ passive: false, capture: true \}\)/);
  assert.match(page, /document\.body\.classList\.contains\("page-detail-open"\)/);
  assert.match(page, /new MutationObserver\(scheduleSynchronization\)/);
  ...
  assert.match(page, /className="dialog-outside-dismiss"/);
  assert.match(css, /\.garden-panel-close\{position:sticky;[^}]*top:0/);
  assert.match(css, /\.nature-lab-close,\.papercut-close,\.space-slice-close,\.modal-close\{position:fixed/);
