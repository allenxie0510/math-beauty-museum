...
test("ships four interactive WebGL halls and twelve concepts", async () => {
  ...
  assert.match(museum, /"MATH BEAUTY MUSEUM", 5\.45, 8\.15, "#f4f2ff", \.96, 0, 2\.36/);
  assert.match(museum, /"数学美学展", 3\.7, 7\.25, "#b8c9ff", 1\.08, 0, 1\.52/);
  ...
  assert.match(museum, /new THREE\.PlaneGeometry\(1\.8, 1\.8\), makeChevronMaterial/);
  assert.doesNotMatch(museum, /const arrowShape = new THREE\.Shape/);
  assert.match(museum, /new THREE\.TubeGeometry\(guideCurve, 260, \.022, 7, false\)/);
  assert.match(museum, /glowMaterial\("#ffe2b6", \.16\)/);
