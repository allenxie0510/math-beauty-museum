...
import type { ConceptCandidate, HometownConceptId } from "../domain/types";
...
export async function prepareHometownImage(file: File): Promise<PreparedImage> {
  ...
  try {
    ...
    const visual = analyzePixels(bitmap);
    ...
    return { display, thumbnail, width: Math.round(bitmap.width * scale), height: Math.round(bitmap.height * scale), candidates: [...byId.values()].sort((a, b) => b.confidence - a.confidence).filter((item) => item.confidence >= .55).slice(0, 3) };
  } finally {
    bitmap.close();
  }
}
...
function analyzePixels(bitmap: ImageBitmap): ConceptCandidate[] {
  const size = 96;
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
