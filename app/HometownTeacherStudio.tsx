...
export function HometownTeacherStudio({ close, preview }: { close: () => void; preview: (slug: string, manifest?: import("./hometown-math/domain/types").HometownSceneManifest) => void }) {
  ...
  const saveExhibit = async (patch: Partial<TeacherExhibitDraft> = {}) => {
    ...
    setBusy(true);
    try {
      await request(`/api/hometown/exhibits/${selected.id}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ conceptId: next.conceptId, title: next.title, interpretation: next.interpretation, evidence: next.evidence, overlay: next.overlay, zoneId: next.zoneId, order: next.order, teacherConfirmed: next.teacherConfirmed }) });
      updateSelectedLocal(patch);
  ...
  const chooseConcept = (conceptId: HometownConceptId) => {
    ...
    updateSelectedLocal({ conceptId, title: concept.shortTitle, interpretation: concept.childExplanation, overlay: concept.overlay, teacherConfirmed: false });
  };
...
function Inspector({ item, draft, busy, update, chooseConcept, save }: { item: TeacherExhibitDraft; draft: TeacherExhibitionDraft; busy: boolean; update: (patch: Partial<TeacherExhibitDraft>) => void; chooseConcept: (id: HometownConceptId) => void; save: (patch?: Partial<TeacherExhibitDraft>) => void }) {
  ...
  return <div className="studio-inspector-form"><img src={item.imageUrl} alt={item.filename}/><span>原始文件 · {item.filename}</span><h3>{candidates.length ? "发现了这些可能的数学" : "暂未发现清晰数学"}</h3>{candidates.length ? <div className="studio-candidates">{candidates.map((candidate) => <button key={candidate.conceptId} className={item.conceptId === candidate.conceptId ? "active" : ""} onClick={() => chooseConcept(candidate.conceptId)}><span>{candidate.labelZh}<small>{candidate.evidence}</small></span><b>{Math.round(candidate.confidence * 100)}%</b></button>)}</div> : <p className="studio-no-math">这不是失败。可以从下方 Registry 手动选择有确切证据的概念，也可以不把它放进本次展览。</p>}<label>受控数学概念<select value={item.conceptId ?? ""} onChange={(event) => chooseConcept(event.target.value as HometownConceptId)}><option value="" disabled>请选择</option>{CONCEPT_REGISTRY.map((concept) => <option key={concept.id} value={concept.id}>{concept.labelZh} · {concept.labelEn}</option>)}</select></label><label>儿童标题（最多 18 字）<input maxLength={18} value={item.title} onChange={(event) => update({ title: event.target.value })}/></label><label>一句话解读<textarea value={item.interpretation} onChange={(event) => update({ interpretation: event.target.value })}/></label><label>画面证据<textarea value={item.evidence} onChange={(event) => update({ evidence: event.target.value })}/></label><label>所属分区<select value={item.zoneId} onChange={(event) => update({ zoneId: event.target.value })}>{draft.zones.map((zone) => <option key={zone.id} value={zone.id}>{zone.name}</option>)}</select></label><div className="studio-confirm"><button onClick={() => save()} disabled={busy}>保存修改</button><button className={item.teacherConfirmed ? "confirmed" : ""} onClick={() => save({ teacherConfirmed: !item.teacherConfirmed })} disabled={busy || !item.conceptId}>{item.teacherConfirmed ? "已由教师确认 ✓" : "确认概念与解读"}</button></div></div>;
}
