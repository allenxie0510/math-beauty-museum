...
import { createAdaptiveResolutionController, getAutoRenderProfile, startAdaptiveResolutionMonitor } from "./adaptive-rendering";
...
type GardenItem = {
  ...
  explanation: string;
  color: string;
  controls: Array<{ key: string; symbol: string; label: string; min: number; max: number; step: number; suffix?: string }>;
};
...
const GARDEN_ITEMS: GardenItem[] = [
  ...
  },
  {
    ...
    color: "#8fd052", controls: [
      ...
      { key: "treeAngle", symbol: "α", label: "分枝角度", min: 14, max: 42, step: 1, suffix: "°" },
      { key: "treeRatio", symbol: "r", label: "枝条比例", min: 0.58, max: 0.78, step: 0.01 },
    ],
  },
  {
    ...
    color: "#8d73e8", controls: [
      ...
      { key: "butterflyScale", symbol: "s", label: "翅膀大小", min: 0.7, max: 1.35, step: 0.01 },
      { key: "butterflyGap", symbol: "d", label: "镜像距离", min: 0.35, max: 0.85, step: 0.01 },
    ],
  },
  {
    ...
    color: "#8fcf45", controls: [
      ...
      { key: "vineRadius", symbol: "R", label: "展开半径", min: 0.35, max: 0.95, step: 0.01 },
      { key: "vineHeight", symbol: "H", label: "生长高度", min: 1.8, max: 4, step: 0.05 },
    ],
  },
  {
    ...
    color: "#ff9c63", controls: [
      ...
      { key: "buildingHeight", symbol: "H", label: "纵向伸展", min: 0.8, max: 2.2, step: 0.05 },
      { key: "buildingRadius", symbol: "S", label: "结构尺度", min: 0.65, max: 1.25, step: 0.01 },
    ],
  },
  {
...
function formatGardenControlValue(control: GardenItem["controls"][number], value: number) {
  const decimals = control.step < .01 ? 3 : control.step < .1 ? 2 : control.step < 1 ? 1 : 0;
  return `${value.toFixed(decimals)}${control.suffix ?? ""}`;
}
...
function MathGardenCanvas({ selectedId, onSelect, settings, audioAnalyserRef, pondPlayingRef }: { selectedId: GardenId | null; onSelect: (id:GardenId)=>void; settings: GardenSettings; audioAnalyserRef:{current:AnalyserNode|null}; pondPlayingRef:{current:boolean} }) {
  ...
  useEffect(()=>{
    ...
    try{renderer=new THREE.WebGLRenderer({antialias:!lowPower,alpha:false,powerPreference:lowPower?"low-power":"high-performance"});}catch(error){console.error("Math garden WebGL initialization failed",error);container.dataset.webglError="true";return;}
    const applyPixelRatio=(pixelRatio:number)=>{renderer.setPixelRatio(pixelRatio);renderer.setSize(container.clientWidth,container.clientHeight,false);container.dataset.pixelRatio=pixelRatio.toFixed(2)};
    ...
    const contextLost=(event:Event)=>{event.preventDefault();container.dataset.webglReady="false";container.dataset.webglError="true"};const contextRestored=()=>{container.dataset.webglReady="true";container.dataset.webglError="false"};
    ...
    const stopAdaptiveResolution=startAdaptiveResolutionMonitor(adaptiveResolution,()=>isSceneVisible&&!document.hidden);
    ...
    const resize=()=>{if(!container.clientWidth||!container.clientHeight)return;camera.aspect=container.clientWidth/container.clientHeight;camera.updateProjectionMatrix();renderer.setPixelRatio(adaptiveResolution.pixelRatio);renderer.setSize(container.clientWidth,container.clientHeight,false)};const stopObservingResize=observeElementSize(container,resize);
    return()=>{cancelAnimationFrame(frame);timer.dispose();stopAdaptiveResolution();stopObservingResize();stopObservingVisibility();renderer.domElement.removeEventListener("pointerdown",down);renderer.domElement.removeEventListener("pointerup",up);renderer.domElement.removeEventListener("webglcontextlost",contextLost);renderer.domElement.removeEventListener("webglcontextrestored",contextRestored);controls.dispose();disposeObject(scene);renderer.dispose();renderer.domElement.remove();delete container.dataset.webglReady;delete container.dataset.webglError;delete container.dataset.quality;delete container.dataset.pixelRatio;delete container.dataset.measuredFps;sceneGroups.clear()};
  ...
  return <div className="garden-webgl" ref={host}><div className="webgl-fallback" role="status" aria-live="polite"><b className="webgl-loading-copy">正在开启数学花园</b><span className="webgl-loading-copy">数学生命体正在生长</span><b className="webgl-error-copy">3D 花园暂时没有开启</b><span className="webgl-error-copy">请开启图形加速，或点击下方按钮再次尝试。</span><button className="webgl-retry" type="button" onClick={()=>setRetryKey(key=>key+1)}>重新开启 3D</button></div></div>;
}
...
export function MathGardenWorld({ onProgress }: { onProgress:(count:number)=>void }) {
  ...
  const stopPondSound=useCallback((updateState=true)=>{
    ...
    pondAudioContext.current=null;
    if(context&&context.state!=="closed")void context.close();
    if(updateState)setPondPlaying(false);
  },[]);
  const startPondTrack=useCallback(async(nextTrackId:PondTrackId)=>{
    stopPondSound();
    const track=POND_TRACKS.find(entry=>entry.id===nextTrackId)??POND_TRACKS[0];
    try{
